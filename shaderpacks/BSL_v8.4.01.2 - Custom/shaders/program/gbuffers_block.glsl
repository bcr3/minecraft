...
void main() {
	...
	#if MC_VERSION >= 11300
	float skipParallax = float(blockID == 400);
	#else
	...
	if(blockID == 401) albedo.a = 0.0;

	if (albedo.a > 0.001) {
		vec2 lightmap = clamp(lmCoord, vec2(0.0), vec2(1.0));
		
		float metalness       = 0.0;
		float emission        = float(blockID == 205);
		...
		if (blockID == 205 && dot(color.rgb, vec3(1.0)) > 2.66) {
		...
		if(blockID == 205) albedo.a = sqrt(albedo.a);
		#endif
	}
